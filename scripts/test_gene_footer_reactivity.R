...
source('global.R')
if (nzchar(Sys.getenv("CGV_TEST_STATE_HELPERS"))) sys.source(Sys.getenv("CGV_TEST_STATE_HELPERS"),.GlobalEnv)
...
shiny::testServer(server_fun, {
    footer_key_calls <- character()
    footer_records <- list()
    original_footer <- build_footer_content_ui
    assign('build_footer_content_ui', function(...) {
        a <- list(...)
        result <- original_footer(...)
        footer_records[[length(footer_records)+1L]] <<- list(id=a$plot_id,context=a$context,
            total_transcripts=a$total_transcripts,html=htmltools::renderTags(result)$html)
        result
    },envir=environment(original_footer))
    original_key <- build_footer_cache_key
    assign('build_footer_cache_key', function(...) {
        a <- list(...)
        footer_key_calls <<- c(footer_key_calls, paste(a$context,a$id_local))
        original_key(...)
    }, envir=environment(original_key))
    ...
    cat('FOOTER_FIRST',paste(footer_key_calls,collapse='|'),'\n')
    ...
    cat('FOOTER_SECOND',paste(footer_key_calls,collapse='|'),'\n')
    old_count <- length(footer_key_calls)
    start <- proc.time()[['elapsed']]
    for (i in 1:10) {
        seqs <- isolate(genSequencesHomologous())
        seqs[['unrelated_probe']] <- paste0('unrelated_',i)
        genSequencesHomologous(seqs)
        session$flushReact()
    }
    cat('UNRELATED_KEY_CALLS',length(footer_key_calls)-old_count,'elapsed_ms',1000*(proc.time()[['elapsed']]-start),'\n')
    stopifnot(model_preparations == 2L)
    keyed <- is.function(attr(genSequencesHomologous,'read_key'))
    stopifnot(length(footer_key_calls)-old_count == if(keyed) 0L else 20L)
    # Own metadata must still update this card and preserve the other footer.
    other_before <- sum(footer_key_calls=='homo 3')
    meta <- isolate(plotGeneMetaHomologous())
    meta[['1']]$total_transcripts <- 3L
    plotGeneMetaHomologous(meta); session$flushReact()
    stopifnot(tail(footer_records,1)[[1]]$id=='1',tail(footer_records,1)[[1]]$total_transcripts==3L)
    if(keyed) stopifnot(sum(footer_key_calls=='homo 3')==other_before)
    meta[['1']]$total_transcripts <- 2L
    plotGeneMetaHomologous(meta); session$flushReact()
    stopifnot(tail(footer_records,1)[[1]]$total_transcripts==2L)
    saveRDS(footer_records,paste0(args[[2]],'.footers'))
    cat('OWN_METADATA_UPDATE_OK\n')
